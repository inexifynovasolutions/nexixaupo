...
// Version: v5 (Phase C — Environment Variables)
// ============================================================

// 🔐 ENV VARS — Auto-loaded from env-config.js (generated by build.js)
// Production: Vercel env vars se aate hain
// Local: .env file se aate hain
const SUPABASE_URL = (window.NEXI_ENV && window.NEXI_ENV.SUPABASE_URL) || '';
const SUPABASE_KEY = (window.NEXI_ENV && window.NEXI_ENV.SUPABASE_ANON_KEY) || '';

// ⚠️ Safety check — agar env vars load nahi hue to warn karein
if (!SUPABASE_URL || !SUPABASE_KEY) {
    console.error('❌ CRITICAL: Supabase credentials not loaded!');
    console.error('   - Production: Check Vercel env vars');
    console.error('   - Local: Check .env file + run "node build.js"');
    console.error('   - Also: Ensure <script src="env-config.js"></script> is loaded BEFORE supabase-config.js');
}
...
function initSupabase() {
    ...
    if (!SUPABASE_URL || !SUPABASE_KEY) {
        console.error('❌ Cannot init Supabase — missing credentials');
        return null;
    }
...
async function getActiveAlerts(target = 'both') {
    ...
    if (error) { console.error('getActiveAlerts error:', error); return []; }
...
async function getAllAlerts() {
    ...
    if (error) { console.error('getAllAlerts error:', error); return []; }
    return data || [];
}
...
async function createAlert(alertData) {
    ...
    if (error) { console.error('createAlert error:', error); return { error }; }
    return { data };
}
...
async function updateAlert(id, updates) {
    ...
    const { data, error } = await sb
        ...
        .select()
        .single();
    if (error) { console.error('updateAlert error:', error); return { error }; }
    return { data };
}
...
async function deleteAlert(id) {
    ...
    if (error) { console.error('deleteAlert error:', error); return { error }; }
    return { success: true };
}
...
async function getAllTemplates(limit = 100) {
    ...
    const { data, error } = await sb
        ...
        .order('created_at', { ascending: false })
        .limit(limit);
    if (error) { console.error('getAllTemplates error:', error); return []; }
    return data || [];
}

async function createTemplate(templateData) {
    const sb = initSupabase();
    const { data, error } = await sb.from('alert_templates').insert([templateData]).select().single();
    if (error) { console.error('createTemplate error:', error); return { error }; }
    return { data };
}
...
async function updateTemplateUsage(id, currentCount) {
    ...
    const { data, error } = await sb
        .from('alert_templates')
        .update({ times_used: (currentCount || 0) + 1, last_used_at: new Date().toISOString() })
        .eq('id', id)
        .select()
        .single();
    if (error) { console.error('updateTemplateUsage error:', error); return { error }; }
    return { data };
}

async function deleteTemplate(id) {
    const sb = initSupabase();
    const { error } = await sb.from('alert_templates').delete().eq('id', id);
    if (error) { console.error('deleteTemplate error:', error); return { error }; }
    return { success: true };
}
...
async function getAllBanners() {
    ...
    if (error) { console.error('getAllBanners error:', error); return []; }
    return data || [];
}
...
async function createBanner(bannerData) {
    ...
    if (error) { console.error('createBanner error:', error); return { error }; }
    return { data };
}
...
async function updateBanner(id, updates) {
    ...
    const { data, error } = await sb
        ...
        .select()
        .single();
    if (error) { console.error('updateBanner error:', error); return { error }; }
    return { data };
}
...
async function deleteBanner(id) {
    ...
    if (error) { console.error('deleteBanner error:', error); return { error }; }
    return { success: true };
}
...
async function getBannerTemplates(limit = 100) {
    ...
    const { data, error } = await sb
        ...
        .order('created_at', { ascending: false })
        .limit(limit);
    if (error) { console.error('getBannerTemplates error:', error); return []; }
    return data || [];
}

async function saveBannerTemplate(templateData) {
    const sb = initSupabase();
    const { data, error } = await sb.from('banner_templates').insert([templateData]).select().single();
    if (error) { console.error('saveBannerTemplate error:', error); return { error }; }
    return { data };
}
...
async function updateBannerTemplateUsage(id, currentCount) {
    ...
    const { data, error } = await sb
        ...
        .select()
        .single();
    if (error) { console.error('updateBannerTemplateUsage error:', error); return { error }; }
    return { data };
}

async function deleteBannerTemplate(id) {
    const sb = initSupabase();
    const { error } = await sb.from('banner_templates').delete().eq('id', id);
    if (error) { console.error('deleteBannerTemplate error:', error); return { error }; }
    return { success: true };
}
...
const RATE_LIMIT_PRESETS = {
    login:          { max: 5, window: 60, block: 900 },
    register:       { max: 3, window: 300, block: 1800 },
    password_reset: { max: 2, window: 600, block: 3600 },
    resend_email:   { max: 1, window: 300, block: 1800 },
    payment_submit: { max: 3, window: 1800, block: 3600 },
    contact_form:   { max: 2, window: 600, block: 3600 }
};
...
async function logVisit(page = 'index', ipAddress = null) {
    try {
        ...
        const payload = { page: page, user_agent: ua, visited_date: today };
        if (ipAddress) payload.ip_address = ipAddress;

        const { data, error } = await sb.from('visits').insert([payload]).select().single();
        if (error) { console.error('logVisit error:', error); return { error }; }
        return { data };
    } catch (err) {
...
async function getTodayVisits() {
    try {
        ...
        if (!sb) return 0;
        const today = new Date().toISOString().split('T')[0];
        const { count, error } = await sb.from('visits').select('*', { count: 'exact', head: true }).eq('visited_date', today);
        if (error) { console.error('getTodayVisits error:', error); return 0; }
        return count || 0;
    } catch (err) {
...
async function getVisitsGroupedByDate(limit = 30) {
    try {
        const sb = initSupabase();
        if (!sb) return [];
        const { data, error } = await sb.from('visits').select('visited_date, ip_address').order('visited_date', { ascending: false }).limit(5000);
        if (error) { console.error('getVisitsGroupedByDate error:', error); return []; }
        ...
        return Object.keys(grouped).sort().reverse().slice(0, limit).map(date => ({
            stat_date: date,
...
async function getDailyStats(limit = 30) {
    try {
        const sb = initSupabase();
        if (!sb) return [];
        const { data, error } = await sb.from('daily_stats').select('*').order('stat_date', { ascending: false }).limit(limit);
        if (error) { console.error('getDailyStats error:', error); return []; }
        return data || [];
    } catch (err) {
...
async function saveDailyStats(statsData) {
    try {
        const sb = initSupabase();
        if (!sb) return { error: 'Supabase init failed' };
        const payload = { ...statsData, updated_at: new Date().toISOString() };
        const { data, error } = await sb.from('daily_stats').upsert([payload], { onConflict: 'stat_date' }).select().single();
        if (error) { console.error('saveDailyStats error:', error); return { error }; }
        return { data };
    } catch (err) {
...
async function getOverallWinRate(days = 30) {
    try {
        const sb = initSupabase();
        if (!sb) return '0%';
        const { data, error } = await sb.from('daily_stats').select('win_trades, total_trades').order('stat_date', { ascending: false }).limit(days);
        if (error) { console.error('getOverallWinRate error:', error); return '0%'; }
...
async function getActiveSignals(target = 'clients', limit = 10) {
    try {
        const sb = initSupabase();
        if (!sb) return [];
        let query = sb.from('signals').select('*').eq('is_active', true).order('created_at', { ascending: false }).limit(limit);
        ...
        const { data, error } = await query;
        if (error) { console.error('getActiveSignals error:', error); return []; }
        return data || [];
    } catch (err) {
...
async function getAllSignals() {
    try {
        ...
        if (error) { console.error('getAllSignals error:', error); return []; }
        return data || [];
    } catch (err) {
...
async function getTotalSignalsCount() {
    try {
        const sb = initSupabase();
        if (!sb) return 0;
        const { count, error } = await sb.from('signals').select('*', { count: 'exact', head: true });
        if (error) { console.error('getTotalSignalsCount error:', error); return 0; }
        return count || 0;
    } catch (err) {
...
async function getActiveClientsCount() {
    try {
        const sb = initSupabase();
        if (!sb) return 0;
        const { count, error } = await sb.from('users').select('*', { count: 'exact', head: true }).eq('status', 'active');
        if (error) { console.error('getActiveClientsCount error:', error); return 0; }
        return count || 0;
    } catch (err) {
...
async function getTotalClientsCount() {
    try {
        ...
        if (error) { console.error('getTotalClientsCount error:', error); return 0; }
        return count || 0;
    } catch (err) {
...
async function getTradesBetween(startISO, endISO) {
    try {
        const sb = initSupabase();
        if (!sb) return [];
        const { data, error } = await sb.from('trades').select('profit, exit, created_at')
            .gte('created_at', startISO).lte('created_at', endISO).not('exit', 'is', null);
        if (error) { console.error('getTradesBetween error:', error); return []; }
        return data || [];
    } catch (err) {
...
// VERSION LOG
// ============================================================
console.log('✅ supabase-config.js v5 loaded — ENV + Auth + Legacy + RateLimit + Session');

// ============================================================
// SESSION TIMEOUT SYSTEM (B6)
// ============================================================
